...
@ThreadSafe
public class MDriver extends DatabaseDriver {
     ...
     * @deprecated use the other {@linkplain com.facebook.stetho.inspector.database.SqliteDatabaseDriver#SqliteDatabaseDriver(Context,
     * DatabaseFilesProvider) constructor} and pass in the {@linkplain DefaultDatabaseFilesProvider
     * default provider}.
     */
    @Deprecated
    public MDriver(Context context) {
     ...
     */
    public MDriver(Context context, DatabaseFilesProvider databaseFilesProvider) {
        super(context);
    ...
    private SQLiteDatabase openDatabase(String databaseName) throws SQLiteException {
        ...
        if (mDatabaseFilesProvider == null) {
            databaseFile = mContext.getDatabasePath(databaseName);

        } else {
            List<File> dataFiles = mDatabaseFilesProvider.getDatabaseFiles();
